use crate::com::{connect, getfilef, receive, send};
use crate::data::PeerConfig;
use crate::db::{
    get_buffermap, get_file, get_peer, get_peer_key, get_peers_from_file, set_buffermap,
};
...
pub fn is_stream_open(stream: &TcpStream) -> bool {
    ...
    match stream.peek(buffer_slice) {
        Ok(_) => true,
        Err(_) => false,
    }
}
...
static FOO_MUTEX: Mutex<()> = Mutex::new(());
...
pub fn get_wanted_piece_from_peer(peer_key: &str, file_key: &str) -> Vec<usize> {
    ...
    let _guard = FOO_MUTEX.lock().unwrap();
    ...
    // TODO How many pieces to get? hardcoded 20 for now
    let nb_pieces: usize = min(50, remaining);
