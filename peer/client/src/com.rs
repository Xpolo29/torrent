...
pub fn receive(expected_answer: &dyn ExpectedAnswer, port: u16, adress: String) -> String {
    let mut stream = TcpStream::connect(format!("{}:{}", adress, port)).unwrap();
...
pub trait ExpectedAnswer {
    fn check_answer(&self, answer:String) -> Result<String, std::io::Error>;
...
impl ExpectedAnswer for ExpectOk {
    fn check_answer(&self, answer: String) -> Result<String, std::io::Error> {
        if answer == "ok\r\n" {
            Ok("Réponse correcte du tracker".to_string())
        } else {
            Err(std::io::Error::new(std::io::ErrorKind::Other, "Erreur réponse du tracker"))
        }
    }
